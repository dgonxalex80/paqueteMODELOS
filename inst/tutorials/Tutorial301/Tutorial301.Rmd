...
```{r p3, exercise=TRUE, exercise.lines = 40}  
library(paqueteMODELOS)
data(corazon)
table(corazon$infarto)
# separacion de muetras
...
test<-corazon[-index_train,]  # muestra de prueba
# oversampling
train.blc <- ovun.sample()
test.blc <- ovun.sample()
...
```{r p4, exercise=TRUE, exercise.lines = 40}  
library(paqueteMODELOS)
library(ROSE)
data(corazon)
table(corazon$infarto)
...
glm(    , family = binomial(link = "logit"), data = train.blc) -> modelo1
...
```{r p5, exercise=TRUE, exercise.lines = 50}  
library(paqueteMODELOS)
library(ROSE)
data(corazon)
table(corazon$infarto)
